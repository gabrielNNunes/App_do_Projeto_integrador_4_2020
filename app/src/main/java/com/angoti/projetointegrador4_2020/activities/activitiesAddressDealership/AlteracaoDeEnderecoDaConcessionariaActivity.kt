...
class AlteracaoDeEnderecoDaConcessionariaActivity : AppCompatActivity() {
...
/*
            ...
            referencia
            */
    private lateinit var idConcessionaria:EditText
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        idConcessionaria = findViewById<EditText>(R.id.et_cadastro_endereco_id)
        idConcessionaria.hint = "Id da Concessionária"
        idConcessionaria.isEnabled = false
    ...
    fun cadastrar(v: View) {
        ...
        RetrofitClient().api.alterarEnderecoConcessionaria(id, "Bearer $tokenAutorizacao", address_DTO).enqueue(object : Callback<Void?> {
            override fun onResponse(call: Call<Void?>, response: Response<Void?>) {
                if (response.code() == 200) {
                    Toast.makeText(this@AlteracaoDeEnderecoDaConcessionariaActivity, "Alteração realizada", Toast.LENGTH_LONG).show();
                    startActivity(Intent(this@AlteracaoDeEnderecoDaConcessionariaActivity, ListagemDeEnderecoDaConcessionariaActivity::class.java))
                }else Toast.makeText(this@AlteracaoDeEnderecoDaConcessionariaActivity, "Falha", Toast.LENGTH_LONG).show();
            }
