...
class CadastroDeEderecoDeUsuarioActivity : AppCompatActivity() {
    ...
    fun cadastrar(view: View) {
        ...
        val idAddress = findViewById<EditText>(R.id.et_cadastro_endereco_id).text.toString().toLong()
        ...
        RetrofitClient().api.cadastraEnderecoUsuario(idAddress, "Bearer $tokenAutorizacao", address).enqueue(object : Callback<AddressDTO> {
            override fun onResponse(call: Call<AddressDTO>, response: Response<AddressDTO>) {
                if (response.code() == 200 || response.code() == 201) {
                    Toast.makeText(this@CadastroDeEderecoDeUsuarioActivity, "Endereço cadastrado com id = " + response.body()?.id, Toast.LENGTH_LONG)
                        .show();
                    startActivity(Intent(this@CadastroDeEderecoDeUsuarioActivity, MainActivity::class.java))
                }else {
                    val mensagem = JSONObject(response.errorBody()?.string())
                    Toast.makeText(this@CadastroDeEderecoDeUsuarioActivity, "Não cadastrado. Erro: ${mensagem.getString("errors")}", Toast.LENGTH_LONG)
                        .show();
            ...
            override fun onFailure(call: Call<AddressDTO>, t: Throwable) {
                Toast.makeText(this@CadastroDeEderecoDeUsuarioActivity, "Erro: " + t.message, Toast.LENGTH_LONG)
                    .show();
