...
class SwipeEnderecoConcessionaria(val addressAdapter: EnderecoConcessionariaAdapter) : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.RIGHT or ItemTouchHelper.LEFT) {

    val background = ColorDrawable(Color.YELLOW)
    val icon = ContextCompat.getDrawable(addressAdapter.contexto, R.drawable.ic_swipe)
