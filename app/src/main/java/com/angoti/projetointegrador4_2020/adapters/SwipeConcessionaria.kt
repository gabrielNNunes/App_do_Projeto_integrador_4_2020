...
class SwipeConcessionaria(val adapter: ConcessionariaAdapter) : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.RIGHT or ItemTouchHelper.LEFT) {

    val background = ColorDrawable(Color.YELLOW)
    val icon = ContextCompat.getDrawable(adapter.contexto, R.drawable.ic_swipe)
